    ...
    public class SecondRepository: Repository<SecondLevel>
    {
        ...
        public void Commit(List<SecondLevel> students)
        {
            ...
            for (int i = 0; i<students.Count; i++)
            {
                ...
                {
                    nws.Cells[j+1, 5].Value = students[i].Filleuls[k].Nom;
                    nws.Cells[j + 1, 6].Value = students[i].Filleuls[k].Telephone;
                    nws.Cells[j + 1, 7].Value = students[i].Filleuls[k].Email;

                    nws.Cells[j + 1, 5, j + 1, 7].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                    nws.Cells[j + 1, 5, j + 1, 7].Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                    nws.Cells[j + 1, 5, j + 1, 7].Style.Border.BorderAround(ExcelBorderStyle.Medium);
                }
