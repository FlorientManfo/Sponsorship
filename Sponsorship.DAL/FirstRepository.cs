...
namespace Sponsorship.DAL
{
    public class FirstRepository: Repository<FirstLevel>
    {
        ...
        public List<FirstLevel> GetALL()
        {
            ...
            {
                var nS = new FirstLevel(ReadCell(i, 0), ReadCell(i, 1), long.Parse(Regex.Replace(ReadCell(i, 2), @"\s", "")), ReadCell(i, 3), File.Exists($@"{imagePath}\" + ReadCell(i, 0) + ".jpg") ? $@"{imagePath}\" + ReadCell(i, 0) + ".jpg" : "../../Resources/unknow.png");
                excelDatas.Add(nS);
                i++;
                item = ReadCell(i, 0);
            }
